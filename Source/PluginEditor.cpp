    ...
    : AudioProcessorEditor(&p), audioProcessor(p)
{
    ...
    setResizeLimits(980, 640, 1440, 900);
    ...
    assistantPanelButton.setColour(juce::TextButton::buttonColourId, controlColour());
    assistantPanelButton.setColour(juce::TextButton::buttonOnColourId, accentColour());
    assistantPanelButton.setColour(juce::TextButton::textColourOffId, textColour());
    assistantPanelButton.setColour(juce::TextButton::textColourOnId, textColour());
    assistantPanelButton.onClick = [this] {
        assistantPanelOpen = ! assistantPanelOpen;
        resized();
        repaint();
    };
    addAndMakeVisible(assistantPanelButton);
...
void SoriMixAudioProcessorEditor::paint(juce::Graphics& g)
{
    ...
    drawAssistantPanel(g, assistantPanelBounds);
...
void SoriMixAudioProcessorEditor::setAssistantPanelVisible(bool shouldBeVisible)
{
    promptBox.setVisible(shouldBeVisible);
    applyButton.setVisible(shouldBeVisible);
    providerBox.setVisible(shouldBeVisible);
    apiKeyBox.setVisible(shouldBeVisible);
    saveKeyButton.setVisible(shouldBeVisible);
    deleteKeyButton.setVisible(shouldBeVisible);
    statusLabel.setVisible(shouldBeVisible);
    credentialLabel.setVisible(shouldBeVisible);
...
void SoriMixAudioProcessorEditor::layoutAssistantPanel(juce::Rectangle<int> bounds)
{
    ...
    {
        assistantPanelButton.setBounds(bounds.reduced(3, 6));
...
void SoriMixAudioProcessorEditor::drawAssistantPanel(juce::Graphics& g, juce::Rectangle<int> bounds)
{
    ...
    g.setColour(assistantPanelOpen ? juce::Colour(0xfffff7f1) : juce::Colour(0xffffefe4));
    ...
    g.setColour(mutedTextColour());
    g.drawFittedText("Prompt the selected module while keeping monitoring visible.", textArea,
...
void SoriMixAudioProcessorEditor::drawCompressorInsight(juce::Graphics& g, juce::Rectangle<float> bounds)
{
    ...
    {
        const auto historyIndex = (reductionHistoryIndex + i) % reductionHistory.size();
        const auto value = juce::jlimit(0.0f, 1.0f, reductionHistory[historyIndex] / 18.0f);
        const auto x = bars.getX() + static_cast<float>(i) * barWidth;
        const auto h = bars.getHeight() * value;
        g.setColour(colour.withAlpha(0.18f + value * 0.62f));
        g.fillRoundedRectangle(juce::Rectangle<float>(x, bars.getBottom() - h, juce::jmax(1.0f, barWidth - 1.0f), h), 2.0f);
    }
...
void SoriMixAudioProcessorEditor::timerCallback()
{
    ...
    reductionHistory[reductionHistoryIndex] = juce::jlimit(0.0f, 18.0f, std::abs(reductionMeter));
    reductionHistoryIndex = (reductionHistoryIndex + 1) % reductionHistory.size();
    repaint();
}
